...
const getInstrumentoXID = (req, res) => new Promise((resolve, reject) => {
    ...
    mysqldb_1.cxMysql.getConnection((err, connection) => {
        ...
        connection.query('SELECT * FROM intrumento WHERE id = ?', [id], (err, results) => {
            if (err)
                console.error(err);
            res.send(results);
        });
...
const crearInstrumento = (req, res) => new Promise((resolve, reject) => {
    ...
    mysqldb_1.cxMysql.getConnection((err, connection) => {
        if (err) {
            ...
            let sql = 'INSERT INTO instrumento(id, instrumento, marca, modelo, imagen, precio, costoEnvio, cantidadVendida, descripcion) VALUES (?,?, ?, ?, ?, ?,?,?,?)';
...
const eliminarInstrumento = (req, res) => new Promise((resolve, reject) => {
    ...
    mysqldb_1.cxMysql.getConnection((err, connection) => {
        ...
        connection.query('DELETE FROM instrumento WHERE id = ?', [id], (err, results) => {
            if (err) {
                console.error(err);
